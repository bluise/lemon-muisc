...
> 配图为 **v1.2.9** 实机截图。日常主流程：**搜索 / 发现 → 下载 → 标签编辑整理**。

### 下载（核心之一）：搜索 · 发现 · 下载管理

多平台关键词检索与批量下载；发现页可粘贴歌单链接或浏览热门歌单；下载页管理任务队列与状态筛选。

![搜索：多平台检索、试听与批量下载](docs/screenshots/search-v1.2.9.png)

![发现：粘贴歌单链接与热门歌单](docs/screenshots/discover-v1.2.9.png)

![下载管理：任务列表与状态筛选](docs/screenshots/download-v1.2.9.png)

### 标签编辑（核心之二）：匹配 · 手动检测 · 重设

扫描本地目录，批量改 ID3 / FLAC；支持匹配缺失 / 选中、**按文件名重设全部**、**手动检测**（标黄不正确标签，可一键采用含封面 / 歌词）。
