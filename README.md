...
- 搜索歌单、详情进度、导入到音乐库歌单
- 酷狗封面补全；导入曲目缺封面时回落到歌单封面

**下载更靠谱**

- 真 FLAC 校验（拒假 flac）；无损默认不降档
- 音质菜单只显示音源真实档位（含酷我 FLAC Hi-Res）
- 同名跳过不误报、移出列表；添加下载后列表实时更新

**体验**

- 应用内确认弹窗；日活统计看板增强（作者站点 PHP 需单独上传）

### v1.2.9 回顾

**标签编辑加强**

- 手机 / 窄屏自适应；手动检测 / 按文件名重设；匹配可暂停停止；可调并发
...
生成 `fpk/lemon-music-1.2.11-x86.fpk` 与 `fpk/lemon-music-1.2.11-arm.fpk`（版本以 `fpk/manifest` 为准）。说明见 [docs/fpk-install.md](docs/fpk-install.md)。
