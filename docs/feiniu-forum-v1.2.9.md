...
左侧选文件夹，中间看文件与批量工具，右侧改标题 / 歌手 / 专辑 / 封面 / 歌词。支持「匹配缺失 / 匹配选中」「按文件名重设」（勾选批量）、「手动检测」。匹配 / 检测过程中可暂停、继续、停止。
...
- **按文件名重设**：勾选后按文件名重新匹配并保存（有确认）  
- **暂停 / 停止**：匹配与手动检测进行中可暂停、继续或停止  
...
| **按文件名重设** | 勾选批量按文件名重刷标签并落盘 |
| **暂停 / 停止** | 匹配与手动检测可中途暂停或停止 |
| **匹配并发** | 有限并行；设置可调 1–6 |
